##---------------------------------------------------------------------------------##
## EPA_plot1.R is 1/6 scripts that visually explore data re: PM2.5 Emissions       ##
...
## the years 1999, 2002, 2005 and 2008 to see if emissions are decreasing.         ##
...
## 4. creates a line plot of all types and the sum total vs time                   ##                                                                                                                                                            
...
EPA_plot1 <- function(){
        ...
        ## sum the emissions for each type and the total per year 
        ## and combine that to create a new table
        
        data.summed.line<-ddply(Data, "year", summarise, 
                           total=sum(Emissions), 
                           point = sum(Emissions[which(type=="POINT")]), 
                           nonpoint = sum(Emissions[which(type=="NONPOINT")]), 
                           onroad = sum(Emissions[which(type=="ON-ROAD")]), 
                           nonroad = sum(Emissions[which(type=="NON-ROAD")])
                           )
        ...
        with(data.summed.line, plot(y=data.summed.line$total, 
                          x=data.summed.line$year, 
                          ylab="PM2.5 Emissions (in tons)", 
                          xlab="Year", xlim=c(1998, 2009), 
                          main="EPA PM2.5 Emission Totals by Source and Year", 
                          ylim=1.1*c(0, max(data.summed.line$total)), 
                          type="o", 
                          col="#86E0AB")
             )
        
        lines(y=data.summed.line$point, x=data.summed.line$year, type="o", col="#DAB5D6")
        lines(y=data.summed.line$nonpoint, x=data.summed.line$year, type="o", col="#E4B570") 
        lines(y=data.summed.line$onroad, x=data.summed.line$year, type="o", col="#89D7DD")
        lines(y=data.summed.line$nonroad, x=data.summed.line$year, type="o", col="#C6DE73")
        
        legend("topright", title="Emission Sources", legend=c("Sum Total", "POINT", "NONPOINT", "ON-ROAD", "NON-ROAD"), 
               xjust=1, cex=0.8, bg="transparent", lwd=1, lty=1, bty="n", col=c("#86E0AB", "#DAB5D6", "#E4B570", "#89D7DD", "#C6DE73"))
        
        ## --------------COPY SCREEN DEVICE to PNG-------------------##
        print("Copying Plot to .png")
        dev.copy(png, file = "EPA_plot1.png", width=720, height=480) ## copy from the screen device to a PNG file
        print("PNG created.")
